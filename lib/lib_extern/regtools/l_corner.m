...
%% start code snippet from Henry Sallandt
if (nargin==8)    
  index = find(rho < M);
  if isempty(index)
    M = input(['M is chosen to be too small. The minimum value is: ' num2str(min(rho)) ' - choose a different M!']);
    index = find(rho < M);
  end
  rho = rho(index); eta = eta(index); reg_param = reg_param(index);
end
%% end code snippet from Henry Sallandt
%%
