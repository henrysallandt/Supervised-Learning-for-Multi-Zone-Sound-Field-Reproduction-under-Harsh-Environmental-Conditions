...
np = 8;                           % Number of identified points.

% Initialization.
if (ps < 1 | ps > 2), error('Illegal value of ps'), end
n = length(rho); ni = round(n/np)-1;
...
if (max(eta)/min(eta) > 10 | max(rho)/min(rho) > 10)
  if (nargin < 5)
    ...
    loglog(rho,eta,marker,rho(2*ni:ni:n),eta(2*ni:ni:n),'x'), axis(ax)
    HoldState = ishold; hold on;
    for k = 2*ni:ni:n
      text(rho(k),eta(k),num2str(reg_param(k), '%.1e'), 'Rotation',45,'HorizontalAlignment','right','Clipping',true);
    end
  ...
  if (nargin < 5)
    ...
    plot(rho,eta,marker,rho(2*ni:ni:n),eta(2*ni:ni:n),'x'), axis(ax)
    HoldState = ishold; hold on;
    for k = 2*ni:ni:n
      text(rho(k),eta(k),num2str(reg_param(k), '%.1e'), 'Rotation',45,'HorizontalAlignment','right','Clipping',true);
    end
...
xlabel('residual norm $||\mathbf{G} \mathbf{w} - \mathbf{p_{goal}}||_2$')
if (ps==1)
  ylabel('solution norm $|| \mathbf{w} ||_2$')
else
